...
// 漢字（\u4e00-\u9faf）、ひらがな、カタカナ、および全角記号などを広くカバーする日本語判定
...
async function fetchRelatedWithFallback(vId) {
    for (const base of APIS) {
        try {
            ...
            // 【修正ポイント1】最新のInvidiousで使われている「data.related」もフォールバックに含める
            const related = data.related || data.relatedVideos || data.recommendedVideos || [];
            
            // 日本語（漢字含む）が含まれる動画のみを抽出
...
export default async function handler(req) {
    ...
    try {
        ...
        const resultIds = finalJapaneseVideos.map(v => v.videoId).filter(Boolean);
        ...
        return new Response(JSON.stringify(["SERVER_ERROR"]), { status: 500 });
    }
}
