...
const Actions = {
        ...
        return items.map((item, index) => {
            ...
            <div class="v-card" onclick="Actions.playFromList(${index})">
                <div class="thumb-container">
                    ...
                    ${snip.liveBroadcastContent === 'live' ? '<div class="live-badge" style="box-shadow: 0 0 10px #ff0000;">● LIVE</div>' : ''}
    ...
    async fetchComments(vId, order = 'relevance') {
        this.commentSortOrder = order;
        const box = document.getElementById('comments-box');
        if (!box) return;
        box.innerHTML = `<div style="padding:20px; color:#aaa;">コメント読み込み中... (順序: ${order === 'relevance' ? '評価順' : '新着順'})</div>`;
        try {
            const data = await YT.fetchAPI('commentThreads', { videoId: vId, part: 'snippet', maxResults: 50, order: order });
            if (!data.items || data.items.length === 0) { box.innerHTML = '<div style="padding:20px; color:#aaa;">コメントはオフになっているか、ありません。</div>'; return; }
            
            let html = `
                <div style="display:flex; gap:15px; margin-bottom:20px; padding:0 10px;">
                    <button class="btn" onclick="Actions.fetchComments('${vId}', 'relevance')" style="${order==='relevance' ? 'background:#3ea6ff;' : ''}">評価順</button>
                    <button class="btn" onclick="Actions.fetchComments('${vId}', 'time')" style="${order==='time' ? 'background:#3ea6ff;' : ''}">新着順</button>
                </div>
            `;
            html += data.items.map(item => {
                const c = item.snippet.topLevelComment.snippet;
                return `
                <div style="display:flex; gap:12px; margin-bottom:18px; padding:0 10px;">
                    <img src="${c.authorProfileImageUrl}" style="width:36px; height:36px; border-radius:50%;">
                    <div style="font-size:13px;">
                        <div style="font-weight:bold; margin-bottom:2px;">${c.authorDisplayName} <span style="font-weight:normal; color:#aaa; font-size:11px; margin-left:8px;">${timeAgo(c.publishedAt)}</span></div>
                        <div style="line-height:1.4; white-space:pre-wrap; word-break:break-all;">${c.textDisplay}</div>
                        <div style="margin-top:6px; color:#aaa; font-size:11px;">👍 ${c.likeCount || 0}</div>
                    </div>
                </div>`;
            }).join('');
            box.innerHTML = html;
        } catch (e) { box.innerHTML = '<div style="padding:20px; color:#aaa;">コメントを取得できませんでした。</div>'; }
    },
    ...
    async play(video) {
        ...
        if (isShorts) {
            ...
            document.getElementById('view-container').innerHTML = `
                <div class="watch-layout">
                    <div class="player-area">
                        <div class="video-wrapper" id="video-wrapper"><iframe src="${YT.getEmbedUrl(vId)}" style="width:100%; height:100%; border:none;" allowfullscreen allow="autoplay"></iframe></div>
                        <div style="margin-top:15px; display:flex; gap:10px; align-items:center; background:#1e1e1e; padding:10px 20px; border-radius:10px; flex-wrap:wrap;">
                            ...
                            <button class="btn" onclick="YT.seek(10)">10s ⏩</button>
                            <button id="stream-play-btn" class="btn" style="background:#ff4757; color:white; font-weight:bold;">⚡️ ストリーム再生</button>
                        </div>
                        <div style="padding-top:15px;">
                            ...
                            <div id="comments-section">
                                <h3 style="margin-bottom:20px;">コメント</h3>
                                <div id="comments-box" style="background:#0f0f0f; border-radius:12px; padding:15px 5px;"></div>
                            </div>
                        </div>
                    </div>
            ...
            // ストリーム再生のクリックイベント登録
            const sBtn = document.getElementById('stream-play-btn');
            if (sBtn) {
                sBtn.onclick = async () => {
                    sBtn.innerText = "読込中...";
                    try {
                        const r = await fetch(`/api/m3u8?id=${vId}`);
                        const d = await r.json();
                        if (d.url) {
                            document.getElementById('video-wrapper').innerHTML = `<video id="video-element" controls style="width:100%; height:100%; border-radius:12px; background:#000;"></video>`;
                            M3U8Player.setupPlayer(d.url);
                        } else { alert("ストリーム取得失敗。通常再生をお使いください。"); sBtn.innerText = "⚡️ ストリーム再生"; }
                    } catch(e) { alert("API接続エラー"); sBtn.innerText = "⚡️ ストリーム再生"; }
                };
            }

            this.fetchComments(vId, 'relevance');
