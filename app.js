const YT = {
    // 画像の通り、KahootのAPIからJSON形式でキーを取得する
    async getEducationKey() {
        try {
            ...
            this.KEY = data.key; // "key" フィールドの値をセット
        } catch (e) {
            console.error("Education Key の取得に失敗したわ。");
        }
    },
    KEY: '',
    async fetchAPI(endpoint, params) {
        if (!this.KEY) await this.getEducationKey();
...
const Actions = {
    ...
    async showSubsPage() {
        ...
        else html += subs.map(ch => `<div class="ch-item" onclick="Actions.openChannel('${ch.id}')" style="cursor:pointer; text-align:center;"><img src="${this.channelIcons[ch.id] || ''}" style="width:60px;height:60px;border-radius:50%;"><br><span>${ch.name}</span></div>`).join('');
        html += `</div><h2 style="margin-top:20px;">📅 新着</h2><div id="subs-grid" class="grid"></div></div>`;
        document.getElementById('view-container').innerHTML = html;

        if (subs.length > 0) {
            const data = await YT.fetchAPI('search', { part: 'snippet', type: 'video', q: subs.map(s => s.name).join(' OR '), maxResults: 20 });
    ...
    renderGridWithDelete(items, targetId, listName) {
        document.getElementById(targetId).innerHTML = items.map((item, i) => `
            <div class="v-card">
                ...
                <button style="background:#ff4d4d;color:white;border:none;border-radius:4px;padding:4px;width:100%;margin-top:8px;" onclick="Actions.removeItem('${listName}', '${item.id}')">抜く</button>
            </div>`).join('');
    },
